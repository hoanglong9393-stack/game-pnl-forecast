...
with rendered_tabs[-1]:
    ...
    if f"pnl_res_{cur_proj}" not in st.session_state:
        ...
        for p in ordered_plats:
            ...
            cur_ltv_data = [cur_ltv_row[c] for c in DISPLAY_LTV_COLS]
            
            cur_rr_row = st.session_state[f"rr_{p}_{cur_proj}"].iloc[0]
            cur_rr_data = [cur_rr_row[c] for c in DISPLAY_RR_COLS]

            c1, c2 = st.columns(2)
            with c1:
                fig_rr = go.Figure()
                fig_rr.add_trace(go.Scatter(x=DISPLAY_RR_COLS, y=cur_rr_data, mode='lines+markers', name=f"KH Hiện Tại ({cur_proj})", line=dict(width=4, dash='dash', color='#D97706')))
                for s in selected_samples:
                    if p in st.session_state.sample_projects[s]:
                        s_rr = st.session_state.sample_projects[s][p]["RR"]
                        s_rr_filtered = s_rr[s_rr["Ngày"].isin(DISPLAY_RR_COLS)]
                        fig_rr.add_trace(go.Scatter(x=s_rr_filtered["Ngày"], y=s_rr_filtered["RR (%)"], mode='lines', name=s, line=dict(width=2)))
                fig_rr.update_layout(title=f"Retention Rate (%) - {p}", height=350, margin=dict(l=20, r=20, t=40, b=20), hovermode="x unified")
                st.plotly_chart(fig_rr, use_container_width=True)

            with c2:
                fig_ltv = go.Figure()
                fig_ltv.add_trace(go.Scatter(x=DISPLAY_LTV_COLS, y=cur_ltv_data, mode='lines+markers', name=f"KH Hiện Tại ({cur_proj})", line=dict(width=4, dash='dash', color='#22C55E')))
                for s in selected_samples:
                    if p in st.session_state.sample_projects[s]:
                        s_ltv = st.session_state.sample_projects[s][p]["LTV"]
                        s_ltv_filtered = s_ltv[s_ltv["Ngày"].isin(DISPLAY_LTV_COLS)]

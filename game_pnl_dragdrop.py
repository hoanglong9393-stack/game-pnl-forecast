...
def login():
    st.markdown("### 🔐 Đăng Nhập Hệ Thống Dự Phóng P&L Game")
    ...
    with col1:
        ...
        if st.button("Đăng nhập", type="primary"):
            if username in USER_DATABASE and USER_DATABASE[username]["password"] == password:
                st.session_state.logged_in_user = USER_DATABASE[username]
    ...
    with col2:
        ...
        * **Marketing (Sửa Traffic & OB 30 ngày):** `mkt_lead` / `123`
        * **Product (Sửa LTV Curves):** `product_lead` / `123`
        * **Viewer (Chỉ xem P&L):** `bod_viewer` / `123`
        """)
...
# ==========================================
with st.sidebar:
    st.write(f"👤 **{current_user['name']}** (`{current_user['role']}`)")
...
# ==========================================
tabs_to_show = []
if current_user["can_edit_traffic"] or current_user["role"] == "admin":
    tabs_to_show.extend(["🤖 1. Traffic Android", "🍎 2. Traffic iOS"])
if current_user["can_edit_ltv"] or current_user["role"] == "admin":
    tabs_to_show.extend(["📈 3. LTV Android", "🍏 4. LTV iOS"])
if current_user["can_view_pnl"] or current_user["role"] == "admin":
    tabs_to_show.append("📊 5. Báo Cáo P&L Tổng Hợp")

rendered_tabs = st.tabs(tabs_to_show)

# TAB 1: ANDROID
if "🤖 1. Traffic Android" in tabs_to_show:
    t_idx = tabs_to_show.index("🤖 1. Traffic Android")
    with rendered_tabs[t_idx]:
        ...
        edited_tr_adr = st.data_editor(
            ...
            num_rows="dynamic" if current_user["can_edit_traffic"] else "fixed",
            disabled=not current_user["can_edit_traffic"],
        ...
        with st.expander("📅 Chi Tiết Phân Bổ 30 Ngày Tháng OPEN BETA (Android) - [Tùy Chỉnh Lệch Đầu / Launch Curve]", expanded=False):
            ...
            edited_ob_adr = st.data_editor(
                ...
                num_rows="fixed", disabled=not current_user["can_edit_traffic"],
...
# TAB 2: IOS
if "🍎 2. Traffic iOS" in tabs_to_show:
    t_idx = tabs_to_show.index("🍎 2. Traffic iOS")
    with rendered_tabs[t_idx]:
...
# TAB 3: LTV ANDROID
if "📈 3. LTV Android" in tabs_to_show:
    t_idx = tabs_to_show.index("📈 3. LTV Android")
    with rendered_tabs[t_idx]:
        ...
        edited_ltv_adr = st.data_editor(
            ...
            num_rows="dynamic" if current_user["can_edit_ltv"] else "fixed",
            disabled=not current_user["can_edit_ltv"],
...
# TAB 4: LTV IOS
if "🍏 4. LTV iOS" in tabs_to_show:
    t_idx = tabs_to_show.index("🍏 4. LTV iOS")
    with rendered_tabs[t_idx]:
...
if "📊 5. Báo Cáo P&L Tổng Hợp" in tabs_to_show:
    t_idx = tabs_to_show.index("📊 5. Báo Cáo P&L Tổng Hợp")
    with rendered_tabs[t_idx]:
